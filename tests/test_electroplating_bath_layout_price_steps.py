...
_BATCH10_DIMS_MM = (80.0, 30.0, 30.0)  # with 20 mm clearance -> best packing is 10 pcs
...
def test_bath_layout_packs_ten_parts_and_eleventh_requires_second_batch(monkeypatch):
    ...
    assert layout_10["geometric_capacity"] == 10
    assert layout_10["batch_capacity"] == 10
    assert layout_10["batch_quantity"] == 10
    assert layout_10["batch_count"] == 1
    assert layout_10["batch_quantity_limited_by"] == "requested_quantity"

    assert layout_11["geometric_capacity"] == 10
...
def test_order_labor_has_step_when_quantity_crosses_one_bath_capacity(monkeypatch):
    ...
    one_batch_process_labor = 1.18 * params_10["operation_time_min"]
    ...
    assert params_11["order_labor_time_min"] == pytest.approx(
        2 * one_batch_process_labor + 11 * mount_time
    )

    # The money calculation is proportional to per-part labor time multiplied
    # by quantity.  Therefore this order-level labor jump is the same structural
    # jump that the final order price must show after cost coefficients are applied.
    ...
    assert actual_increment == pytest.approx(one_batch_process_labor + mount_time)
    assert actual_increment > 5 * params_10["labor_time_min"]
...
def test_batch_count_steps_at_every_capacity_boundary(monkeypatch, batch_capacity, completed_batches):
    ...
    assert before["geometric_capacity"] == batch_capacity
    ...
    assert after["geometric_capacity"] == batch_capacity
...
@pytest.mark.parametrize("completed_batches", [1, 2, 5, 10])
def test_order_labor_jumps_at_every_new_bath_load(monkeypatch, batch_capacity, completed_batches):
    """Order-level labor must increase by one full operation cycle at every boundary."""
    monkeypatch.setattr(ep, "get_defaults", lambda: {"clearance_mm": 0.0, "mount_unmount_time_min": 0.0})
    operation_time_min = 60.0
    one_batch_operation_labor = 1.18 * operation_time_min
    q_before = batch_capacity * completed_batches
    q_after = q_before + 1
    ...
    assert after["order_labor_time_min"] == pytest.approx(
        (completed_batches + 1) * one_batch_operation_labor
    )
    assert after["order_labor_time_min"] - before["order_labor_time_min"] == pytest.approx(
        one_batch_operation_labor
    )
    assert after["labor_formula_effective_n"] == pytest.approx(
        q_after / (completed_batches + 1)
    )
