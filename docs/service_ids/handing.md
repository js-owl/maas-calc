...
`Слесарные работы`. Этот service id относится к `constants.OTHER_SERVICES` и принимается через `/calculate-price` как заявка на ручную оценку производством.
...
3. Так как `service_id="handing"` находится в `OTHER_SERVICES_LIST`, endpoint сразу формирует `UnifiedCalculationResponse`.
