...
from models.calculation_models import CNCMillingCalculationRequest
from models.response_models import UnifiedCalculationResponse

logger = logging.getLogger(__name__)


class CNCMillingCalculator(BaseCalculator):
    """CNC Milling price calculator"""
    
    def __init__(self):
        super().__init__()
        self.service_id = "cnc-milling"
        self.calculation_method = "CNC Milling Price Calculation"
    
    async def calculate(self, request: CNCMillingCalculationRequest) -> UnifiedCalculationResponse:
        """Calculate CNC milling price"""
        self._validate_request(request)
        self._log_calculation_start(request.file_id, "CNC milling")
        
        try:
            # Import calculation functions from legacy main
            from calculations.cnc import calculate_cnc_milling_price
            ...
            result = calculate_cnc_milling_price(calc_params)
            ...
            self._log_calculation_complete(request.file_id, "CNC milling")
            return UnifiedCalculationResponse(**response_data)
            
        except HTTPException as e:
            logger.error(f"Error in CNC milling calculation for file_id {request.file_id}: {e}")
            raise e
